...
export function AllPropertiesTable({
  ...
  const [expandedHotels, setExpandedHotels] = useState<Set<string>>(new Set(['Swiss-Belinn Si...']))
  ...
  const getStatusColor = (status: string) => {
    ...
    if (status === "Closed") return "text-red-500 dark:text-red-400"
    return "text-green-600 dark:text-green-400"
  }

  const getRateColor = (rateData: any) => {
    if (rateData.status === "--" || rateData.status === "Closed") {
      return getStatusColor(rateData.status)
    }
    
    // For numeric rates, use rateType
    if (rateData.rateType === 'highest') {
      return "text-red-500 dark:text-red-400"
    } else if (rateData.rateType === 'cheapest') {
      return "text-green-600 dark:text-green-400"
    } else {
          ...
          <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
            {hotels.map((hotel, index) => (
              <React.Fragment key={hotel.id}>
                <tr className={`hover:bg-blue-50 dark:hover:bg-blue-900/20 ${expandedHotels.has(hotel.name) ? 'bg-blue-50 dark:bg-blue-900/20' : ''}`}>
                        ...
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <div 
                              ...
                              onClick={() => toggleHotelExpansion(hotel.name)}
                            >
                              {expandedHotels.has(hotel.name) ? (
                          ...
                          <TooltipContent className="bg-black text-white border-black text-xs px-2 py-1">
                            <p>{expandedHotels.has(hotel.name) ? "Collapse" : "Expand"}</p>
                          </TooltipContent>
                        </Tooltip>
                      </TooltipProvider>
                            ...
                            <TooltipTrigger asChild>
                              <span className="text-blue-600 hover:text-blue-800 font-medium text-sm cursor-pointer">
                                {(() => {
                                  ...
                                  return hotel.name.length > maxLength 
                       ...
                       <td key={date} className={`px-2 py-3 text-center border-r border-gray-200 dark:border-gray-700 ${visibleDayNames[index] === 'Sat' || visibleDayNames[index] === 'Sun' ? 'bg-blue-50 dark:bg-blue-900/20' : ''}`} style={{ width: '80px', minWidth: '50px', maxWidth: '80px' }}>
                         {rateData.status === '--' ? (
                           <div className="flex flex-col gap-0.5">
                             ...
                             <span className="text-xs text-gray-500 dark:text-gray-400">
                               {rateData.rate > 0 ? rateData.rate.toLocaleString('en-US') : ''}
                             </span>
                           </div>
                         ) : (
                         ...
                         <td key={date} className={`px-2 py-3 text-center border-r border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 ${visibleDayNames[index] === 'Sat' || visibleDayNames[index] === 'Sun' ? 'bg-blue-50 dark:bg-blue-900/20' : ''}`} style={{ width: '80px', minWidth: '50px', maxWidth: '80px' }}>
                           <div className="relative" style={{ height: '180px' }}>
                             ...
                             {index === 3 && (() => {
                               ...
                               const useK = globalMaxRate < 1000000 // If max rate is less than 1M, use K
                               const divisor = useK ? 1000 : 1000000
                               const suffix = useK ? 'K' : 'M'
                             ...
                             <div className={`relative h-full ${index === 3 ? 'ml-3' : ''}`}>
                               ...
                               {(() => {
                                 ...
                                 const rangeData = {
                                   ...
                                   minRate: rateData.minRate || rateData.rate * 0.8, // Default to 80% of current if not provided
                                   maxRate: rateData.maxRate || rateData.rate * 1.2, // Default to 120% of current if not provided
                                 ...
                                 if (rangeData.rateType === 'highest') {
                                   // For highest rate (red), position at top of range
                                   currentPosition = rangeHeight - 4 // Near the top
                                 } else if (rangeData.rateType === 'cheapest') {
                                   // For cheapest rate (green), position at bottom of range
                                   currentPosition = 4 // Near the bottom
                                 } else {
                                   // For normal rate (black), position in middle
                                   ...
                                   <Tooltip>
                                     ...
                                     <TooltipContent className="bg-black text-white border-black text-xs px-3 py-2 max-w-xs">
                                       <div className="space-y-1">
                                         ...
                                         <div className="text-gray-300">(+2 more)</div>
                                       </div>
                                     </TooltipContent>
                                   </Tooltip>
                                 </TooltipProvider>
